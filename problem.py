...
def evaluate_p1(
    ...
    V = sm.compute_velocities(S, start_xyz, cfg.Tf)
    v_norm = np.linalg.norm(V, axis=1)

    # ---- 通信项 R_bar ----
    dc = sm.dc_uav_to_user(S, user_xy)
    ...
    CRB = sm.crb_xyz_sum(Hov, target_hat_xyz, cfg)
    ...
    area_violation = (
        ...
        + np.maximum(-z + cfg.z_min, 0.0).sum()
        + np.maximum(z - cfg.z_max, 0.0).sum()
    )
